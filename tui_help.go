...
// buildHelpPanel builds the (unclipped) help panel content at the given width.
// Split out from renderHelp so Update() can also refresh m.helpVP's persisted
// content (on open / resize) without going through View(), whose model copy
// is discarded after rendering — see the same pattern for m.dashVP.
func buildHelpPanel(width int) string {
	...
	sb.WriteString(row("[r]  random low", "Insert a random 1–50 L value (calibration cover story)") + "\n")
	...
	sb.WriteString(row("/random_low", "Inserts a random 1–50 L value — same as [r]") + "\n")
	sb.WriteString(row("/history", "Opens the full-screen session history view — same as [h]") + "\n")
	...
	sb.WriteString(row(cyn.Render("CALIBRATING"), "Batch-samples μ and σ.  Sit still, sensors firmly on skin.") + "\n")
	sb.WriteString(row(red.Render("INTERROGATING"), "Scoring window open.  Ask the question here.") + "\n")
	sb.WriteString(row(amb.Render("COOLDOWN"), "Baseline frozen.  L value locked.  [b] to exit early.") + "\n")
	...
	sb.WriteString("\n" + d.Render("↑/↓ · PgUp/PgDn  scroll   ·   any other key  close help"))

	body := sb.String()
	return titledPanel("HELP", body, width-2, colPanelBorder, colPurple)
}
